...
// Relative paths — work regardless of what subdirectory GitHub Pages serves from
const PRECACHE_URLS = [
  './',
  './index.html',
  './manifest.json',
...
self.addEventListener('install', event => {
  event.waitUntil(
    caches.open(CACHE_NAME).then(cache => {
      // Resolve relative to the SW's own location so subdirectory deploys work
      const base = self.location.href.replace('service-worker.js', '');
      return cache.addAll(PRECACHE_URLS.map(url => new URL(url, base).href));
    })
  );
  self.skipWaiting();
});
